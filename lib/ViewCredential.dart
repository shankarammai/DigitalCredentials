...
class _ViewCredentialState extends State<ViewCredential> {
  ...
  void initState() {
    ...
    dynamic _getDetails = _load_Details().whenComplete(() {
      ...
      var sharedKey_enc = credentialDocumentJson['credentialData']['encryptionKey'];
  ...
  _show_full_qr(){
   ...
   //random String
   String verifierEnckey=String.fromCharCodes(List.generate(16, (index) => Random().nextInt(33) + 89));
   final encrypter = cryptolib.Encrypter(cryptolib.AES(
       cryptolib.Key.fromUtf8(verifierEnckey),
       mode: cryptolib.AESMode.cbc));
   final iv = 'ThisIsASecuredBlock'.substring(0, 16);
   String enc_Credential= encrypter.encrypt(credentialData,iv: cryptolib.IV.fromUtf8(iv)).base64;

   dynamic credentailDetails={'data':enc_Credential};//,'encryptionKey':verifierEnckey,'encryptionType':'AES-128-CBC'};

   credentialData_map['credentialData']= credentailDetails;

   sendData.addAll(credentialData_map);
   print(sendData['credentialData']);

   CollectionReference showCredentials = FirebaseFirestore.instance.collection('ShowCredential');
   showCredentials.add(sendData).then((value) => {
   Navigator.of(context).push(MaterialPageRoute<Null>(
   builder: (BuildContext context) {
     Map<String, dynamic> dataForQR={'decryptionKey':verifierEnckey,'documentId':value.id};
     print(jsonEncode(dataForQR));
   return AlertDialog(
   title: Text('Scan this QR'),
   content: Container(
          ...
          builder: (
            ...
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              if (snapshot.hasError) {
                ...
                // credentialDataJson = json.decode((this
                //         .credentialData
                //         .substring(1, this.credentialData.length - 1))
                //     .toString());
